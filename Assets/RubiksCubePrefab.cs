...
public class RubiksCubePrefab : MonoBehaviour {
    ...
    public List<List<List<CubePrefab>>> cubeRef;
    public float spacing = 1.05f;

    // Use this for initialization
    void Start () {
        cubePrefabMatrix = new List<List<List<GameObject>>>();
        cubeRef = new List<List<List<CubePrefab>>>();

        for (int x = 0; x < 3; x++)
        {
            List<List<GameObject>> GOPlane = new List<List<GameObject>>();
            List<List<CubePrefab>> CubePlane = new List<List<CubePrefab>>();
            for (int y = 0; y < 3; y++)
            {
                List<GameObject> GORow = new List<GameObject>();
                List<CubePrefab> CubeRow = new List<CubePrefab>();
                for (int z = 0; z < 3; z++)
                {
                    ...
                    CubePrefab temp = cubePrefab.GetComponent<CubePrefab>();
    ...
    List<List<CubePrefab>> getCubePrefabXFace(int r)
    {
        List<List<CubePrefab>> face = new List<List<CubePrefab>>();
        for (int i = 0; i < 3; i++)
        {
            List<CubePrefab> row = new List<CubePrefab>();
    ...
    List<CubePrefab> getOutline(List<List<CubePrefab>> face)
    {
        //converts a 2d matrix of cubes to a linear list of the outline cubes of the provided face
        //the list is ordered in a clockwise direction, starting with the lower left cube [0][0]
        //getOutline returns a list of references to the origional matrix
        List<CubePrefab> outline = new List<CubePrefab>();
        for (int i = 0; i < 3; i++)
        {
            outline.Add(face[0][i]);
        }
    ...
    public void rotateFrontFace(bool clockwise)
    {
        ...
        for (int j = 0; j < iterations; j++)
        {
            ...
            List<CubePrefab> currentFrontOutline = getOutline(getCubePrefabXFace(0));
            for (int i = 0; i < 8; i++)
            {
    ...
    public void rotateBackFace(bool clockwise)
    {
        ...
        for (int j = 0; j < iterations; j++)
        {
            ...
            List<CubePrefab> currentFrontOutline = getOutline(getCubePrefabXFace(2));
            for (int i = 0; i < 8; i++)
            {
